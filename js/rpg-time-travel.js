/**
 * RPG Plugin: Time Travel Confidant
 * mIRC LLM Simulator
 *
 * Plugin for RPGManager. Implements the meta-narrative layer where perceptive
 * personas detect temporal inconsistencies in user speech and become
 * "time travel confidants" — asking questions about the future from 1999.
 *
 * Registers itself with RPGManager at the bottom of this file.
 *
...
const RPG = {
    ...
    handleUserMessage(message) {
        ...
        if (timeElapsedMin >= settings.serverTimeMinutes && 
            ...
            // Notify RPGManager so it routes future messages to this plugin exclusively
    ...
    reset() {
        ...
        // Notify RPGManager we are no longer active
...
const PrivateMessaging = {
    ...
    receiveMessage(nickname, message) {
        ...
        // Route through RPGManager so it dispatches to whichever plugin is active
        if (window.RPGManager) {
            RPGManager.handlePrivateMessage(nickname, message);
        }
    },
...
// Thin adapter that exposes the RPGManager plugin contract.
// All heavy logic lives in the RPG / ConfidantManager objects above.
// ============================================================================

const TimeTravelPlugin = {
    name: 'TimeTravelConfidant',
    ...
    onComplete() {
        // RPGManager.clearActivePlugin() is already called inside RPG.reset()
        RPG.reset();
    },

    // --- Debug / console helpers (delegate to RPG internals) ---
    showState()              { RPG.showState(); },
    debugTrigger()           { RPG.debugTrigger(); },
    triggerApproachNow(t)    { RPG.triggerApproachNow(t); },
    reset()                  { RPG.reset(); },

    // --- Passthrough references for console inspection ---
    get state()          { return RPG.state; },
    get confidant()      { return ConfidantManager.confidant; },
    get stage()          { return ConfidantManager.stage; }
};

// Register with the RPG Manager (order matters: rpg-manager.js must load first)
if (window.RPGManager) {
    RPGManager.register(TimeTravelPlugin);
} else {
    console.error('[rpg-time-travel] RPGManager not found — ensure rpg-manager.js loads before rpg-time-travel.js');
}

// Subsystem references on RPG for backward compatibility
...
// Global exports
// window.RPG kept for backward-compat console debugging (RPG.showState(), etc.)
window.RPG = RPG;
window.TimeTravelPlugin = TimeTravelPlugin;
window.ConfidantManager = ConfidantManager;
window.PrivateMessaging = PrivateMessaging;
window.PromptBuilder = PromptBuilder;
window.QuestionPool = QuestionPool;

console.log('[rpg-time-travel] Plugin loaded and registered with RPGManager');
